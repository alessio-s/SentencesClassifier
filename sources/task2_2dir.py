...
print('max_sequence length:', len(data[0]))
...
left.add(LSTM(1, return_sequences=True, dropout_W=dropout, dropout_U=dropout))

right = Sequential()
right.add(embedding_layer)
right.add(LSTM(1, return_sequences=True, go_backwards=True, dropout_W=dropout, dropout_U=dropout))

model = Sequential()
model.add(Merge([left,right], mode='concat', concat_axis=1))
model.add(LSTM(lstm_units, return_sequences=False, dropout_W=dropout, dropout_U=dropout))
...
#save model
model.save('./noDense_cue_word_detection_model_bidirectional.h5')
model.save_weights('./noDense_cue_word_detection_weights_bidirectional.hdf5')
# Save on file
with open('noDense_batch_size'+ str(b_size) +'_dropout'+str(dropout*100)+ '_lstm_units'+str(lstm_units)+'_dense_units'+str(dense_units)+ '.pickle', 'w') as f:

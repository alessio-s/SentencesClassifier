...
model.add(LSTM(1, return_sequences=True, dropout_W=dropout, dropout_U=dropout))
...
#save model
model.save('./noDense_cue_word_detection_model_Onedirectional.h5')
model.save_weights('./noDense_cue_word_detection_weights_Onedirectional.hdf5')
# Save on file
with open('OnenoDense_batch_size'+ str(b_size) +'_dropout'+str(dropout*100)+ '_lstm_units'+str(lstm_units)+'_dense_units'+str(dense_units)+ '.pickle', 'w') as f:
